...
class S100HDiskTests {
  ...
  def testReadConsist(): Unit = {
    ...
    funit.current_sector = 1
    funit.current_track = 0
    funit.seek()
    ...
    unit.current_track = 0
    unit.current_sector = 1
    ...
    val a2 = unit.byteBuffer.array()
    assertTrue(s"Should have read 137 bytes", a1.length == 137)
    assertTrue(s"Should have read 137 bytes (2)", a2.length == 137)
    ...
    assertTrue(s"Position should be the same after both reads. ${fposition} ${position}", fposition == position)
    Utils.outln("\n\rDone checking read.")
